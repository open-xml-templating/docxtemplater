...
function errorVerifier(e, type, expectedError) {
	...
	if (e.properties.errors) {
		...
		for (let i = 0; i < e.properties.errors.length; i++) {
			const suberror = e.properties.errors[i];
			const cleaned = cleanError(
				suberror,
				expectedError.properties.errors[i]
			);
			const jsonified = jsonifyError(cleaned);
			e.properties.errors[i] = jsonified;
		}
	}
...
function setExamplesDirectory(ed) {
	...
	if (fs && fs.writeFileSync) {
		const fileNames = [];
		const walkResults = walk(examplesDirectory);
		for (const f of walkResults) {
			fileNames.push(f.replace(examplesDirectory + "/", ""));
		}
		fs.writeFileSync(

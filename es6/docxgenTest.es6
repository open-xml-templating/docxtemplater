...
var shouldBeSame = function(zip1,zip2){
	...
	return (() => {
		...
		for (var i in zip1.files) {
			...
			result.push(expect(zip1.files[i].asText()).to.be.equal(zip2.files[i].asText(), "Content differs"));
		}
		return result;
	})();
};
	...
	describe("DocxGenLoading", function() {
		...
		describe("content_loading", function() {
			...
			return it("should load the right content for the document", function() {
				var fullText=(docX['imageExample.docx'].getFullText()); //default value document.xml
...
<w:t>{key}</w:t>`;
	...
	describe('Change the nullGetter', function(){
		return it('should work with null', function() {
			var content= "<w:t>Hello {name}</w:t>";
			var scope= {};
			var parser= function(tag) { return "null"; };
	...
	describe("errors", function(){
		...
		return describe('internal errors', function(){
			return it('should fail', function(){
				...
				var test=
					{fn(){return new XmlTemplater(1,{fileTypeConfig:FileTypeConfig.docx,}); }};
	...
	describe('Complex table example' , function() {
		...
		it('should work with more complex table', function() {
			...
			var fullText=docX["tableComplexExample.docx"].getFullText(); //apply them
